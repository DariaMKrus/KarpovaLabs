{                          Лабораторная работа N 6
             Решение линейной системы методом простой итерации
...
for i:=1 to n do read(rm,x0[i]);
    ...
    writeln(t);
    writeln(t,'  Решение линейной системы методом простой итерации');
    ...
    writeln(t); z:=x0; k:=0; z1:=x;
    for i:=1 to n do
        for j:=1 to n do z1[i]:=z1[i]+a1[i,j]*z[j];
    while nr(z,z1)>=eps do
        begin k:=k+1; z:=z1; z1:=x;
        for i:=1 to n do
            for j:=1 to n do z1[i]:=z1[i]+a1[i,j]*z[j];
        proverka(a,z1,f,fs);
